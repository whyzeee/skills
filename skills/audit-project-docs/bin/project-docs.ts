...
type Diagnostic = { level: 'warning' | 'error'; path?: string; message: string };
type Parsed = { metadata: Record<string, string | string[]>; body: string };
...
function scalar(value: string): string | null {
  ...
  if (singleQuoted || doubleQuoted) {
    ...
    if (trimmed.length < 3 || trimmed.at(-1) !== quote || !["'", '"'].includes(quote)) return null;
    const inner = trimmed.slice(1, -1);
    if ((quote === "'" && inner.includes("'")) || (quote === '"' && /["\\]/.test(inner))) return null;
    return inner;
  }
...
function parseFrontmatter(text: string, rel: string, diagnostics: Diagnostic[]): Parsed | null {
  ...
  for (let index = 1; index < end; index += 1) {
    ...
    if (key === 'tags') {
      ...
      if (rest === '[]') {
        ...
        while (index + 1 < end && /^  - /.test(lines[index + 1])) {
          ...
          if (tag === null) diagnostics.push({ level: 'error', path: rel, message: 'invalid tag' });
          else tags.push(tag.trim());
        }
        if (tags.length === 0) diagnostics.push({ level: 'error', path: rel, message: 'tags must be a block list or []' });
        metadata.tags = tags;
      } else {
    ...
    if (value === null) diagnostics.push({ level: 'error', path: rel, message: `invalid scalar for "${key}"` });
    else metadata[key] = value;
  }

  for (const key of KEYS) {
    if (!(key in metadata)) diagnostics.push({ level: 'error', path: rel, message: `missing frontmatter key "${key}"` });
  }
...
function validateTable(kind: string, section: string, lines: string[], rel: string, diagnostics: Diagnostic[]): void {
...
function validateDocument(rel: string, parsed: Parsed, diagnostics: Diagnostic[]): void {
  const metadata = parsed.metadata;
  const kind = typeof metadata.type === 'string' ? metadata.type : '';
  if (!TYPES.has(kind)) diagnostics.push({ level: 'error', path: rel, message: `invalid document type "${kind}"` });
  for (const key of ['title', 'description']) {
    if (typeof metadata[key] !== 'string' || !metadata[key]) diagnostics.push({ level: 'error', path: rel, message: `empty "${key}"` });
  }
  ...
  if (kind === 'domain' && rel !== 'docs/DOMAIN.md') diagnostics.push({ level: 'error', path: rel, message: 'domain document must be docs/DOMAIN.md' });
  if (kind === 'structure' && rel !== 'docs/app/STRUCTURE.md') diagnostics.push({ level: 'error', path: rel, message: 'structure document must be docs/app/STRUCTURE.md' });
  ...
  for (const heading of expected) {
    if (!substantive(found.bodies[heading] || [])) diagnostics.push({ level: 'error', path: rel, message: `empty section "${heading}"` });
  }

  if (kind === 'interface' || kind === 'logic') {
    for (const heading of ['Within This Category', 'Across Categories']) {
      if (!(found.bodies.Relationships || []).some((line) => line.trim() === `## ${heading}`)) diagnostics.push({ level: 'error', path: rel, message: `missing H2 "${heading}"` });
    }
  }
  if (kind === 'data') {
  ...
  for (const section of Object.keys(TABLES[kind] || {})) {
    if (section === 'body') validateTable(kind, section, lines, rel, diagnostics);
    else validateTable(kind, section, found.bodies[section] || [], rel, diagnostics);
  }
  if (kind === 'decision' || kind === 'issue') {
    ...
    if (values.length !== 1 || !allowed.includes(values[0])) diagnostics.push({ level: 'error', path: rel, message: `invalid ${kind} status "${values.join(' ')}"` });
  }
}
...
type PlannedWrite = { path: string; rel: string; content: Uint8Array };
  ...
  const visit = (directory: string): boolean => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        if (rel.toLowerCase().endsWith('.markdown')) return false;
...
function normalizeMetadata(text: string): string {
  ...
  if (!parsed || diagnostics.length || end < 0) return withoutBom;
  for (let index = 1; index < end; index += 1) {
  ...
  for (let index = 0; index < lines.length; index += 1) parts[index * 2] = lines[index];
  return parts.join('');
}

function replaceStructureBody(text: string, table: string, generatedEol: '\n' | '\r\n'): string | null {
  const withoutBom = text.replace(/^\uFEFF/, '');
  const parts = withoutBom.split(/(\r\n|\n)/);
  const lines = parts.filter((_, index) => index % 2 === 0);
  const end = lines.indexOf('---', 1);
...
function atomicReplace(write: PlannedWrite): void {
...
function inspect(root: string): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
...
function fix(root: string): number {
  let gitRoot: string;
  try {
